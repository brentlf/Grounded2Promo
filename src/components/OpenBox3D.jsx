...
export default function OpenBox3D({ size, half, opened, onOpen, onDragStart, showGlow }) {
  ...
  return (
    <div style={{ transformStyle: 'preserve-3d' }}>
      ...
      {/* front wall — folds down when open */}
      <motion.div
        className="absolute cardboard-texture border border-[#5a4030]/40 overflow-hidden"
        style={{
          width: size,
        ...
        animate={opened ? { rotateX: -70 } : { rotateX: 0 }}
      ...
      <motion.div
        ...
        animate={opened ? { rotateX: 155 } : { rotateX: 90 }}
      ...
      {!opened && (
        <motion.p
          className="absolute left-1/2 text-gold text-[10px] font-bold whitespace-nowrap pointer-events-none"
          style={{
            top: -36,
            transform: `translateX(-50%) translateZ(${innerHalf + 30}px)`,
          }}
          animate={{ y: [0, -3, 0] }}
